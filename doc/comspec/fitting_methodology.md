# Spectrum-fitting methodology — formalism and scientific context
...
covered, and the empirical error column._
...
Band $g$-factors scale with the solar flux, and for CO also with the comet's heliocentric radial
velocity $v_h$ (Swings effect):
...
values. At 70 K and 1 au: H₂O $\nu_3$ 3.14e-4, $\nu_1$ 2.9e-5, $\nu_2+\nu_3-\nu_2$ 2.85e-5,
$\nu_1+\nu_3-\nu_1$ 1.9e-5, the 2.8–3.0 µm hot bands ~1e-5 in total, $\nu_3-\nu_2$ 6.6e-6,
$\nu_1-\nu_2$ 4.4e-6; CO₂ $\nu_3$ 2.71e-3 plus 1.0e-4 in hot bands; CO $v(1\!-\!0)$ 1.92e-4 at
$v_h = 0$. $s_b(v_h) = g(v_h)/g(0)$ is 1 for every band except CO $v(1\!-\!0)$, whose lines coincide
with the solar CO Fraunhofer lines at rest: $s$ rises from 1 at $v_h=0$ to 1.20 at 5 km/s and
1.27–1.31 for $|v_h| \ge 10$ km/s (`co_swings.csv`, interpolated in $v_h$ and $T_{\rm rot}$). Because
one band dominates CO, $s$ is a scalar per channel and multiplies the geometric amplitude, so the
model stays linear in $Q$. $v_h$ of every pointing is $\mathrm{d}r_h/\mathrm{d}t$ from the ephemeris
$r_h(t)$ in the photometry (`dataio.heliocentric_velocity`; agrees with JPL Horizons rates to 0.002 km/s typically, 0.05 km/s at worst).

The previous representation, kept as `profile_source = "gaussian"` (variant `dc_main_gauss`), was
eight bands tabulated in `config.BANDS` (Ootsubo et al. 2012 Table 2):
...
Both are **optically thin** excitation models with no collisional excitation and no
radiation-trapping correction. The database templates include the non-resonant cascade (which is
what produces the hot bands); `T_rot` (default 70 K) selects the template, i.e. the distribution of
intensity among the lines of a band — the band-integrated $g$ change by < 3 % over 30–130 K.
...
In the current model there is no separate list of bands: the species template is the database
spectral density $g_\lambda(\lambda; T_{\rm rot})$ [photons s⁻¹ molecule⁻¹ µm⁻¹], rebinned onto the
model grid (flux-conserving, `fluorescence.rebin_density`) and multiplied by the photon energy
$hc/\lambda$ cell by cell, so $\sum_b g_b\,\Phi_b(\lambda)\,hc/\lambda_b \to g_\lambda(\lambda)\,hc/\lambda$.
The Gaussian `band_profile()` of the legacy model survives only for `profile_source = "gaussian"`.
...
$$\mathcal{S}_X(\lambda) \;=\; g_{\lambda,X}(\lambda; T_{\rm rot})\,\frac{hc}{\lambda}\; \lambda_{\mu m}^2 \frac{10^{23}}{c}\quad[\mathrm{mJy\ per\ unit\ amplitude}],
\qquad \mathcal{A}_X(r_h,\Delta,v_h) \;=\; \frac{N_{\rm ap}}{Q}\,\frac{s_X(v_h)}{r_h^2}\,\frac{1}{4\pi\Delta^2}.$$

Each species contributes one geometry-independent template times one scalar (the CO Swings factor
$s_{\rm CO}(v_h)$ is part of the scalar). That is precisely the structure of a linear model with
...
### 7.6 Revisions of 2026-09-12

- **Generalised least squares.** The continuum model of a band is a polynomial whose
  coefficient covariance $C$ is saved with every fit; the emission channels of that band share
  its uncertainty through $V C V^{\mathsf T}$, $V_{ik} = (\lambda_i - \lambda_c)^k$. The solve now
  uses the full covariance $\Sigma = \mathrm{diag}(\sigma_{\rm phot}^2) + \sum_b V_b C_b V_b^{\mathsf T}$
  (converted between the continuum's flux space and the solve's space with the per-channel
  distance-correction factors) by Cholesky whitening: $\hat Q = (A^{\mathsf T}\Sigma^{-1}A)^{-1}A^{\mathsf T}\Sigma^{-1}y$,
  $\chi^2 = r^{\mathsf T}\Sigma^{-1}r$. The diagonal of $\Sigma$ is exactly the error the diagonal
  solve used, so §10.2a is resolved; the `dc_main_diag` variant keeps the diagonal solve for
  comparison.
- **Tiers.** `detected` requires $Q/\sigma_Q \ge 3$; `marginal` covers $1 \le Q/\sigma_Q < 3$ with the
  value reported and a 3σ limit quoted; `upper_limit` below 1σ (§10.2b resolved; the robust
  counts still add $n_{\rm eff} \ge 2$).
- **No negative-channel cut** (§10.2c): every channel enters the solve.
- **Hot-band cap**: the H₂O hot-band fallback is offered only inside 3 au.
- **Windows and continuum** (2026-09-14, method matrix, `pipeline_decisions.md` §7.9): 2.7 µm
  emission 2.55–2.80 µm over a 2.30–3.00 µm continuum, CO₂ continuum 3.90–4.65 µm, one-sided
  windows extended to 1 µm from the band edge, polynomial order per fit by leave-one-out CV.
- **Aperture and grouping**: one fixed aperture per phase (20 000 km inside 3 au, 40 000 km
  beyond, 60 000 km when under 1.5 px; 2026-09-14) inside the physical 150 000 km sky annulus;
  Δ spread < 20 % inside every group.  The negative tail of the fits (≤ −2σ) matches the
  Gaussian expectation with the GLS errors and exceeds it 3–6× with diagonal errors.
